...
std::shared_ptr<TreeNode> Parser::parseConstDecl() {
    ...
    node->addChild(std::make_shared<TreeNode>(NodeType::TERMINAL, peek().line, "const"));
...
std::shared_ptr<TreeNode> Parser::parseVarDecl() {
    ...
    if (!match("SEMICN")) {
        // 使用带同步恢复的错误处理
        error2(semicolonExpectedLine, "i");
    }
...
std::shared_ptr<TreeNode> Parser::parseStmt() {
    ...
    if (check("LBRACE")) {//检测到大括号，就调入语句块处理
        ...
        if (!match("LPARENT")) {
            error(printfLine, "j");
            syncTo({"STRCON", "RPARENT"});
        }
         */
        match("LPARENT");
        /*
        if (!match("STRCON")) {//字符串常量
            error(printfLine, "i");
            syncTo({"COMMA", "RPARENT"});
        }
         */
        match("STRCON");//字符串常量
        //只要匹配到逗号就添加变量节点
...
std::shared_ptr<TreeNode> Parser::parseMulExp() {
    ...
    while (check("MULT") || check("DIV") || check("MOD")) {
        ...
        if (check("MULT")) {
            opNode = std::make_shared<TreeNode>(NodeType::EXP, peek().line);
        } else if (check("DIV")) {
            opNode = std::make_shared<TreeNode>(NodeType::EXP, peek().line);
        } else { // MOD
            opNode = std::make_shared<TreeNode>(NodeType::EXP, peek().line);
