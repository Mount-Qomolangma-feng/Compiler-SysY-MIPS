...
void Lexer::handleOperator() {
    ...
    if (multiSym.count(two)) {
        ...
        if (c == '&' || c == '|') {
            pushToken("ERROR", std::string(1, c));  // 使用ERROR类别码
            pushError(line, "a");
            ++pos;
...
void Lexer::writeTokens(const std::string& filename) const {
    ...
    // 按词法分析识别单词的顺序输出每个单词的信息
    for (const auto& token : tokens) {
        fout << token.code << " " << token.lexeme << "\n";
    }

    fout.close();
    std::cout << "词法分析结果已输出到: " << filename << std::endl;
}
