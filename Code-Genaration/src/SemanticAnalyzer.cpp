...
// [新增实现] 递归遍历 AST，收集初始化列表中的所有表达式
// 忽略 '{', '}', ',' 等结构符号，只抓取 EXP 和 CONST_EXP
void SemanticAnalyzer::collectInitExprNodes(const std::shared_ptr<TreeNode>& node, std::vector<std::shared_ptr<TreeNode>>& outExprNodes) {
    if (!node) return;

    // 如果当前节点本身就是表达式，收集它
    if (node->nodeType == NodeType::EXP || node->nodeType == NodeType::CONST_EXP) {
        outExprNodes.push_back(node);
        return;
    }

    // 否则递归遍历子节点 (处理 InitVal -> '{' InitVal ',' InitVal '}')
    for (const auto& child : node->children) {
        // 过滤掉终结符，只处理相关的非终结符
        if (child->nodeType == NodeType::INIT_VAL ||
            child->nodeType == NodeType::CONST_INIT_VAL ||
            child->nodeType == NodeType::EXP ||
            child->nodeType == NodeType::CONST_EXP) {
            collectInitExprNodes(child, outExprNodes);
        }
    }
}

// [新增实现] 填充数组初始化值
void SemanticAnalyzer::fillArrayInitValues(SymbolEntry& entry, const std::shared_ptr<TreeNode>& initValNode, bool requireConst) {
    if (!entry.isArray() || !initValNode) return;

    std::vector<std::shared_ptr<TreeNode>> exprNodes;
    collectInitExprNodes(initValNode, exprNodes);

    entry.arrayInitValues.clear();

    // 遍历收集到的表达式进行求值
    for (const auto& expr : exprNodes) {
        // 限制数量，防止初始化列表过长导致越界（SysY通常忽略多余的）
        if (entry.arrayInitValues.size() >= entry.arraySize && entry.arraySize > 0) break;

        EvalResult res = evaluateExpression(expr);

        // 如果是全局/静态/Const数组，必须是常量表达式
        if (requireConst && !res.isConstant) {
            // 这里可以记录具体的列号，暂时用 entry.line
            addError(entry.line, "b"); // 或其他适合 "初始化值不是常量" 的错误码
            entry.arrayInitValues.push_back(0); // 出错填0
        } else {
            entry.arrayInitValues.push_back(res.value);
        }
    }

    // 补零：如果初始化值个数少于数组大小，剩余部分填0
    while (entry.arrayInitValues.size() < entry.arraySize) {
        entry.arrayInitValues.push_back(0);
    }
}
...
EvalResult SemanticAnalyzer::evaluateLVal(const std::shared_ptr<TreeNode>& node) {
    ...
    // [修改点 1]：删除了原先 "if (!symbol->isConstant()) return ..." 的粗暴拦截
    // 改为在下方根据具体类型精确判断
    ...
    // 情况 A: 标量常量 (const int a = 5)
    if (symbol->type == SymbolType::ConstInt) {
        // 直接返回常量值
        return EvalResult(symbol->value, true, symbol->type);
    }

    // 情况 B: 数组常量 (const int a[2] = {1, 2})
    if (symbol->type == SymbolType::ConstIntArray) {
        // 如果有下标访问
        if (!expChildren.empty()) {
            ...
            if (indexResult.isConstant) {
                // [关键修改] 使用 getArrayElementValue 获取具体元素值
                // 而不是返回 symbol->value (那是基地址或占位符)
                // getArrayElementValue 内部已处理越界（返回0），此处无需重复检查
                int val = symbol->getArrayElementValue(indexResult.value);
                return EvalResult(val, true, SymbolType::Int); // 元素视为Int
            }
        }
        // 如果没有下标（数组名）或下标不是常量，无法视为编译期常量数值
        return EvalResult(0, false, symbol->type);
    }

    // 情况 C: 普通变量 / 静态变量 (Int, StaticInt, IntArray...)
    // 在常量求值阶段，它们均视为非常量
    return EvalResult(0, false, symbol->type);
}
...
void SemanticAnalyzer::visitConstDef(const std::shared_ptr<TreeNode>& node, bool isGlobal) {
    ...
    // === 计算存储信息 ===
    // 计算占用字节数
    entry.size = entry.getByteSize(); // 使用 SymbolEntry 中的辅助函数

    if (isGlobal) {
        // 全局变量：设置标签 (通常就是变量名，或加前缀)
        entry.label = ident;
        entry.offset = 0; // 全局变量偏移量通常为0
    } else {
        // 局部常量：分配栈空间 (MIPS中常量通常也会分配栈空间，或者直接立即数替换)
        // 为了统一处理，我们分配空间
        entry.offset = currentStackOffset;
        currentStackOffset += entry.size;
        debugPrint("visitConstDef", "Allocated offset: " + std::to_string(entry.offset));
    }
    // ===================
    ...
    if (entry.isArray()) {
        ...
        // 调用辅助函数递归填充数组值，参数 true 表示必须是常量
        fillArrayInitValues(entry, initValNode, true);
    }
    else {
        // 2. 标量常量：解析单个值
        ...
        for (const auto& child : node->children) {
            if (child->nodeType == NodeType::CONST_INIT_VAL) {
                ...
                if (!child->children.empty() && child->children[0]) {
                    // 使用 evaluateExpression 更加通用，它能处理 ConstExp 节点
                    EvalResult result = evaluateExpression(child->children[0]);

                    if (result.isConstant) {
                        entry.value = result.value;
                    } else {
                        // 常量定义必须初始化为常量表达式，否则默认为0 (通常此处已在语义检查中报错)
                        entry.value = 0;
                    }
                } else {
                    entry.value = 0;
...
void SemanticAnalyzer::visitVarDef(const std::shared_ptr<TreeNode>& node, bool isGlobal, bool isStatic) {
    ...
    if (!ident.empty()) {
        ...
        // === 计算存储信息 ===
        entry.size = entry.getByteSize();

        if (isGlobal || isStatic) { // 静态变量也当作全局存储处理
            entry.label = isStatic ? ("static_" + ident + "_" + std::to_string(node->line)) : ident;
            entry.offset = 0;
        } else {
            // 局部变量：分配栈偏移
            entry.offset = currentStackOffset;
        ...
        // =========================================================
        // 变量初始化处理（仅限 全局 / static）
        // 必须正确处理数组和标量，且初始化值必须是常量表达式
        // =========================================================
            ...
            if (entry.isArray()) {
                ...
                if (initValNode) {
                    ...
                    if (entry.arraySize > 0) {
                        entry.arrayInitValues.resize(entry.arraySize, 0);
                    }
                }
            }
            else {
                ...
                if (initValNode && !initValNode->children.empty()) {
                    // INIT_VAL → Exp
                    // 使用 evaluateExpression 以通用支持 Exp 节点
                    EvalResult result = evaluateExpression(initValNode->children[0]);

                    if (result.isConstant) {
                        entry.value = result.value;
                    } else {
                        // 全局 / static 变量初始化必须是常量表达式
                        addError(node->line, "b");
                        entry.value = 0;
                    }
                } else {
...
void SemanticAnalyzer::visitFuncDef(const std::shared_ptr<TreeNode>& node) {
    ...
    if (!funcName.empty()) {
        ...
        // === 重置栈偏移量 ===
        // 每次进入新函数，栈偏移从0开始计算（或者从保留区域大小开始，这里假设为0）
        ...
        // === 回填栈帧大小 ===
        // 函数分析完毕，currentStackOffset 此时包含了所有参数和局部变量的总大小
        // 我们将其更新到符号表中
        symbolTable.updateFuncSymbolSize(funcName, currentStackOffset);
        debugPrint("visitFuncDef", "Function " + funcName + " stack size: " + std::to_string(currentStackOffset));
        // ===================
...
void SemanticAnalyzer::visitMainFuncDef(const std::shared_ptr<TreeNode>& node) {
    ...
    // 【关键修改】: 必须将 main 加入符号表，以便内部维护其栈帧大小等信息
    // 虽然输出时不显示，但内部必须存在
    ...
    // === 回填大小 ===
    symbolTable.updateFuncSymbolSize("main", currentStackOffset);
    // ===============
...
void SemanticAnalyzer::visitFuncFParam(const std::shared_ptr<TreeNode>& node) {
    ...
    if (!paramName.empty()) {
        ...
        // === 计算参数偏移 ===
        // 参数也作为局部变量分配空间
        paramEntry.size = 4; // 参数（int或指针）都是4字节
        paramEntry.offset = currentStackOffset;
        currentStackOffset += paramEntry.size;
        // ==================
