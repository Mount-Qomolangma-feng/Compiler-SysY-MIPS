...
void IRGenerator::visitFuncDef(const std::shared_ptr<TreeNode>& node) {
    ...
    int baseOffset = 12;
    // 【修改点 1】新增：追踪实际使用的最大栈空间 (初始化为 baseOffset)
    int maxUsedOffset = baseOffset;
    ...
    for (const auto& sym : funcScope->getSymbols()) {
        ...
        // 【修改点 2】实时更新最大偏移量
        // 无论语义分析的 stackFrameSize 是否正确，这里直接根据变量实际占用的空间进行统计
        if (entry.offset + entry.size > maxUsedOffset) {
            maxUsedOffset = entry.offset + entry.size;
        }
    ...
    // 【修改点 3】不再依赖可能错误的 funcSym->stackFrameSize，直接使用计算出的 maxUsedOffset
    /* // 原错误代码：
    SymbolEntry* funcSym = lookupSymbol(funcName);
    if (funcSym) {
        // 如果同名局部变量遮蔽了函数名，funcSym->stackFrameSize 会是 0，导致 frameSize = 12
        // 进而导致临时变量覆盖参数区
    ...
    */
    ...
    // 正确代码：直接使用现场计算的准确值
    currentCodeGenInfo->frameSize = maxUsedOffset;

    // 4. 处理函数体
    for (const auto& child : node->children) {
        if (child->nodeType == NodeType::BLOCK) {
            // 必须传入 true，让 Block 复用当前已开启的 Scope (Body Scope)
...
void IRGenerator::visitMainFuncDef(const std::shared_ptr<TreeNode>& node) {
    ...
    // 【修改点 3】不再依赖可能错误的 stackFrameSize，直接使用现场计算的准确值
    /* // 原错误代码：
    ...
    */
    ...
    for (const auto& child : node->children) {
        if (child->nodeType == NodeType::BLOCK) {
            // 必须传入 true，让 Block 复用当前已开启的 Scope
            visitBlock(child, true);
        }
    }
...
void IRGenerator::visitVarDef(const std::shared_ptr<TreeNode>& node) {
    ...
    // ================== [修改点 2: 动态维护 frameSize] ==================
    // 同步局部变量到 CodeGen 表，并【关键】更新 frameSize
    if (currentCodeGenInfo && entry->scope != 1 && !isStaticType) {
        std::string uniqueName = varOp->name;
        ...
        if (!currentCodeGenInfo->symbolMap.count(uniqueName)) {
            ...
            // 【新增关键修复】: 动态撑大 frameSize
            // 如果这个新定义的变量占用了栈顶以上的空间，必须更新 frameSize。
            // 否则后续 newTemp() 会误以为这块空间是空闲的，导致临时变量覆盖局部变量。
            int endOffset = cgEntry.offset + cgEntry.size;
            if (endOffset > currentCodeGenInfo->frameSize) {
                currentCodeGenInfo->frameSize = endOffset;
            }
        }
    }
    // =======================================================================
