...
struct Operand {
    ...
    // 添加默认构造函数
    Operand() : type(OperandType::IMM), value(0), symbol(nullptr) {}
    ...
    // 找到 Operand 结构体中的 toString 方法
    std::string toString() const {
        if (type == OperandType::IMM) return "#" + std::to_string(value);

        // 【修改前】
        // if (type == OperandType::VAR) return symbol->name;

        // 【修改后】
        // 优先使用 Operand 自身的 name，因为它可能包含了 scope 后缀 (如 a_2)
        // 只有当 name 为空时（极少见），才回退到 symbol->name
        if (!name.empty()) return name;
        if (type == OperandType::VAR && symbol) return symbol->name;
