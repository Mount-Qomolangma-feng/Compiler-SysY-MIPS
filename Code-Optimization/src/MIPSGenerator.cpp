...
void MIPSGenerator::generateTextSegment() {
    ...
    for (auto instr : instructions) {
        ...
        switch (instr->op) {
                ...
                // 映射算术指令
            {
                ...
                // === 【新增】移位运算 (强度削减优化) ===
            case IROp::SLL: // 逻辑左移 (Shift Left Logical) <<
            case IROp::SRA: // 算术右移 (Shift Right Arithmetic) >>
            {
                // 优化器生成的格式通常是: result = arg1 << arg2(IMM)
                Operand* shamtOp = instr->arg2;

                // 1. 获取源操作数寄存器
                MipsReg r_src = regAlloc.getReg(instr->arg1);

                // 2. 分配目标寄存器
                MipsReg r_dest = regAlloc.allocateReg(instr->result);

                std::string op = (instr->op == IROp::SLL) ? "sll" : "sra";

                if (shamtOp->type == OperandType::IMM) {
                    // 情况 A: 移位量是立即数 (最常见，如 x << 3)
                    // 指令格式: sll $rd, $rt, sa
                    emit(op + " " + MipsHelper::getRegName(r_dest) + ", " +
                         MipsHelper::getRegName(r_src) + ", " + std::to_string(shamtOp->value));
                } else {
                    // 情况 B: 移位量是变量 (较少见，但为了完备性支持)
                    // 指令格式: sllv $rd, $rt, $rs (注意 MIPS 是 sllv rd, rt, rs)
                    // 对应的 op 变为 sllv 或 srav
                    MipsReg r_shamt = regAlloc.getReg(shamtOp);
                    std::string opv = (instr->op == IROp::SLL) ? "sllv" : "srav";

                    emit(opv + " " + MipsHelper::getRegName(r_dest) + ", " +
                         MipsHelper::getRegName(r_src) + ", " + MipsHelper::getRegName(r_shamt));
                }
            }
                break;
    ...
    // ================== 【新增修改：执行窥孔优化并输出】 ==================
    // 此时所有通过 emit() 生成的指令都存在 asmBuffer 中。
    // 我们现在调用 flushAsmBuffer()，它会：
    // 1. 扫描 asmBuffer 执行窥孔优化 (peepholeOptimize)
    // 2. 将优化后的指令写入 output 文件 (fout)
    // 3. 清空 asmBuffer
    flushAsmBuffer();
    // ======================================================================
}
...
std::string MIPSGenerator::getAddress(Operand* op, MipsReg tempReg) {
    // ================= [DEBUG START] =================
    std::cout << "\n[Addr-Debug] >>> getAddress() called" << std::endl;
    std::cout << "  > Operand Name: \"" << op->name << "\"" << std::endl;
    ...
    std::cout << "  > Operand Type: " << typeStr << std::endl;

    // 2. 打印关联的 Symbol 信息 (如果是 VAR)
    if (op->symbol) {
        std::cout << "  > Symbol Info: Scope=" << op->symbol->scope
                  << ", isParam=" << (op->symbol->isParam ? "true" : "false")
                  << ", isArray=" << (op->symbol->isArray() ? "true" : "false") << std::endl;
    } else {
        std::cout << "  > Symbol Info: nullptr (Temp variable or Literal?)" << std::endl;
    }

    // 3. 打印当前函数上下文
    CodeGenFunctionInfo* currentFuncInfo = getCurrentFuncInfo();
    if (currentFuncInfo) {
        std::cout << "  > Current Context: Function \"" << currentFuncInfo->funcName << "\"" << std::endl;
    } else {
        std::cout << "  > Current Context: nullptr (Global scope or Context lost?)" << std::endl;
    }
    // ================= [DEBUG END] =================
    ...
    if (op->type == OperandType::TEMP || op->type == OperandType::VAR) {
        ...
        if (currentFuncInfo) {
            ...
            if (it != currentFuncInfo->symbolMap.end()) {
                ...
                std::string result = "-" + std::to_string(offset) + "($fp)";

                // [DEBUG]
                std::cout << "  > [HIT] Found in stack frame map. Offset=" << offset << std::endl;
                std::cout << "  > [RETURN] Local Address: " << result << std::endl;

                return result;
            } else {
                // [DEBUG]
                std::cout << "  > [MISS] Not found in current function's symbolMap." << std::endl;
            }
        }

        // 2. 没在栈帧中找到，进行严格的错误检查
        // [DEBUG]
        std::cout << "  > [CHECK] Performing strict scope check..." << std::endl;

        if (op->symbol) {
            // 【新增】判断是否为静态变量
            // 请根据你的 SymbolEntry 定义确认 SymbolType 枚举值的名字
            bool isStatic = (op->symbol->type == SymbolType::StaticInt ||
                             op->symbol->type == SymbolType::StaticIntArray);

            // 【修正】错误条件：(是参数 或 作用域是局部) 且 (不是静态变量)
            // 只有普通的局部变量没在栈里才算错，静态局部变量没在栈里是正常的
            if (!isStatic && (op->symbol->isParam || op->symbol->scope > 1)) {
                std::cerr << "[MIPS Error] CRITICAL: Param/Local variable '" << op->name
                          << "' not found in stack frame!" << std::endl;
                std::cerr << "    Reason: Scope=" << op->symbol->scope
                          << " (>1 means local), isParam=" << op->symbol->isParam
                          << ", isStatic=" << isStatic << std::endl; // Debug info
        ...
        if (op->type == OperandType::TEMP) {
            std::cerr << "[MIPS Error] CRITICAL: Temp variable '" << op->name
                      << "' not found in stack frame!" << std::endl;
            return "0($zero)";
        }

        // 3. 确认为全局变量
        std::cout << "  > [DECISION] Treating as GLOBAL Variable (Scope=1 or Static)." << std::endl;

        std::string regName = MipsHelper::getRegName(tempReg);
        // 【核心修复】优先使用 Label (如 static_map_cnt_19)
        std::string labelName = op->name;
        if (op->symbol && !op->symbol->label.empty()) {
            labelName = op->symbol->label;
        }

        // 使用 labelName 而不是 op->name
        // 【修改前】 emit("la " + regName + ", " + labelName);
        // 【修改后】
        emit("la " + regName + ", " + getMipsLabel(labelName));

        std::string result = "0(" + regName + ")";
        std::cout << "  > [RETURN] Global Address: " << result << std::endl;
        return result;
    }

    std::cout << "  > [WARNING] Operand type is not VAR/TEMP. Returning 0($zero)." << std::endl;
    return "0($zero)";
}
...
MipsReg MIPSGenerator::RegisterManager::getReg(Operand* op, bool isLoad) {
    ...
    // 2. 检查变量是否已在寄存器中 (Cache Hit)
    if (varToReg.count(varName)) {
        MipsReg reg = varToReg[varName];

        // === 【修改点 1：LRU 核心逻辑】 ===
        // 既然这次访问用到了这个寄存器，它就变成了“最近使用的”。
        // 我们调用辅助函数将其移到 busyRegs 队列的末尾。
        makeMostRecentlyUsed(reg);
        // ================================

        return reg;
    }

    // 3. 不在寄存器中，需要分配 (Cache Miss)
    MipsReg reg;
    ...
    if (!freeRegs.empty()) {
        ...
        // 3b. 没有空闲寄存器，执行 LRU 淘汰
        // === 【修改点 2：淘汰逻辑】 ===
        // 因为我们总是把“最近使用的”移到队尾，
        // 所以队首 (front) 的寄存器一定是“最久未被使用”的 (Least Recently Used)。
        // 直接淘汰它即可。
        reg = busyRegs.front();
        busyRegs.pop_front();
...
MipsReg MIPSGenerator::RegisterManager::allocateReg(Operand* result) {
    ...
    // 1. 如果已经分配了寄存器，复用它 (Cache Hit)
    if (varToReg.count(varName)) {
        MipsReg reg = varToReg[varName];

        // === 【修改点 1：LRU 核心逻辑】 ===
        // 该寄存器即将被写入，说明它非常活跃。
        // 调用辅助函数将其移到 busyRegs 队列的末尾。
        makeMostRecentlyUsed(reg);
        // ================================
    ...
    if (!freeRegs.empty()) {
        ...
        // === 【修改点 2：淘汰逻辑】 ===
        // 此时 busyRegs.front() 存放的是“最久未被使用”的寄存器 (LRU)。
        // 也就是队列里唯一一个最近没有被 makeMostRecentlyUsed 刷新过的。
        reg = busyRegs.front();
        busyRegs.pop_front();
        spillReg(reg);
        // ============================
...
// === [新增实现] RegisterManager::getAddrReg ===
MipsReg MIPSGenerator::RegisterManager::getAddrReg(Operand* op) {
    std::string varName = op->name;

    // 1. 如果基地址已经在寄存器中，直接复用 (Cache Hit)
    if (varToReg.count(varName)) {
        MipsReg reg = varToReg[varName];

        // === 【修改点 1：LRU 核心逻辑】 ===
        // 数组基地址被复用，说明它正活跃（比如在循环中）。
        // 必须刷新它到队尾，防止被 FIFO 逻辑误杀。
        makeMostRecentlyUsed(reg);
        // ================================
    ...
    // 2. 如果是“指针类型的变量”（例如函数参数 int arr[]），
    // 它的值本身就是地址，所以直接用 getReg 加载其值即可。
    if (op->type == OperandType::TEMP ||
        (op->symbol && op->symbol->isParam)) {
        // getReg 内部我们已经修改过支持 LRU 了，直接调用即可
        return getReg(op);
    }

    // 3. 此时 op 必定是一个数组符号（全局数组 或 局部栈数组）
    // 我们需要分配一个新寄存器来存放这个“计算出来的地址” (Cache Miss)
    MipsReg reg;
    if (!freeRegs.empty()) {
        reg = freeRegs.front();
        freeRegs.pop_front();
    ...
    varToReg[varName] = reg;
    regToVar[reg] = varName;
    if (op->symbol) varToSymbol[varName] = op->symbol;
    ...
    // 4. 计算基地址放入 reg (替代了原本 visitLoadStore 里的逻辑)
    CodeGenFunctionInfo* funcInfo = generator.getCurrentFuncInfo();

    // 检查是否为局部变量
    if (funcInfo && funcInfo->symbolMap.count(varName)) {
        // === 局部数组 ===
        int stackOffset = funcInfo->symbolMap[varName].offset;
        int size = funcInfo->symbolMap[varName].size;
        // 计算地址：$fp - (offset + size - 4)
        generator.emit("addiu " + MipsHelper::getRegName(reg) + ", $fp, -" +
                       std::to_string(stackOffset + size - 4));
    } else {
        // === 全局数组 ===
        std::string label = varName;
        if (op->symbol && !op->symbol->label.empty()) {
            label = op->symbol->label;
        }
        // 生成 la $reg, _label
        generator.emit("la " + MipsHelper::getRegName(reg) + ", " + generator.getMipsLabel(label));
    }

    // 5. 【关键】标记为 Dirty 吗？
    // NO。虽然我们在寄存器里放了新值，但这个值是“地址常量”，
    // 我们永远不希望 spillReg 把这个地址写回到数组的内存里（那会覆盖数组数据）。
    // 确保它不在 dirty set 中：
    dirtyRegs.erase(reg);
...
void MIPSGenerator::RegisterManager::makeMostRecentlyUsed(MipsReg reg) {
    // 在 busyRegs 列表中查找该寄存器
    // std::list 的查找和移除是 O(N)，但因为寄存器最多只有 8-10 个，速度极快
    for (auto it = busyRegs.begin(); it != busyRegs.end(); ++it) {
        if (*it == reg) {
            // 找到后，将其移除，并重新加入到队尾
            busyRegs.erase(it);
            busyRegs.push_back(reg);
    ...
    // 如果还没在 busyRegs 里（例如刚从 freeRegs 分配），
    // 这里的逻辑通常不需要处理，因为分配时会自动 push_back
}
...
void MIPSGenerator::visitLoadStore(IRInstruction* instr) {
    if (instr->op == IROp::STORE) {
        ...
        // 3. 【核心修改】获取基地址寄存器 (缓存化)
        // 不再硬编码 $t9，而是向 RegisterManager 申请
        // 如果是连续访问，这里会返回同一个寄存器，且不生成新指令
        MipsReg r_base = regAlloc.getAddrReg(baseOp);
        std::string baseRegName = MipsHelper::getRegName(r_base);

        // 4. 生成 Store 指令
        if (offOp->type == OperandType::IMM) {
            // sw $val, offset($base)
            emit("sw " + MipsHelper::getRegName(r_val) + ", " +
                 std::to_string(offOp->value) + "(" + baseRegName + ")");
        }
        else {
            // 此时我们需要计算 最终地址 = $base + $offset
            // 为了不破坏 cached 的 $base 寄存器，我们必须用临时寄存器或者 $t9
            // 这里使用 $t9 是安全的，因为它只活在这一条指令里

            emit("addu $t9, " + baseRegName + ", " + MipsHelper::getRegName(r_offset));
            emit("sw " + MipsHelper::getRegName(r_val) + ", 0($t9)");
        }
    }
        ...
        // 【核心修改】同样使用 getAddrReg
        MipsReg r_base = regAlloc.getAddrReg(baseOp);
        std::string baseRegName = MipsHelper::getRegName(r_base);

        if (offOp->type == OperandType::IMM) {
            emit("lw " + MipsHelper::getRegName(r_dest) + ", " +
                 std::to_string(offOp->value) + "(" + baseRegName + ")");
        }
        else {
            ...
            // 计算地址到 $t9，加载到 r_dest
            emit("addu $t9, " + baseRegName + ", " + MipsHelper::getRegName(r_offset));
            emit("lw " + MipsHelper::getRegName(r_dest) + ", 0($t9)");
        }
    }
    else if (instr->op == IROp::GET_ADDR) {
        // GET_ADDR result, base, offset
        // 目标是把 (base + offset) 的地址存入 result
        ...
        if (offOp->type == OperandType::IMM) {
            if (offOp->value == 0) {
                // 如果 offset 是 0，直接 move
                emit("move " + MipsHelper::getRegName(r_dest) + ", " + baseRegName);
            } else {
                emit("addiu " + MipsHelper::getRegName(r_dest) + ", " + baseRegName + ", " + std::to_string(offOp->value));
            }
        } else {
            MipsReg r_offset = regAlloc.getReg(offOp);
            emit("addu " + MipsHelper::getRegName(r_dest) + ", " + baseRegName + ", " + MipsHelper::getRegName(r_offset));
        }
    }
}
...
void MIPSGenerator::emit(const std::string& asmCmd) {
    // 【修改前】 fout << "    " << asmCmd << "\n";
    // 【修改后】 写入缓冲区，注意保留缩进
    asmBuffer.push_back("    " + asmCmd);
}

void MIPSGenerator::emitLabel(const std::string& label) {
    // 【修改前】 fout << label << ":\n";
    // 【修改后】 统一添加前缀
    // 【修改前】 fout << getMipsLabel(label) << ":\n";
    // 【修改后】 写入缓冲区，Label 通常顶格写
    asmBuffer.push_back(getMipsLabel(label) + ":");
}

// 文件: MIPSGenerator.cpp

void MIPSGenerator::peepholeOptimize() {
    // 遍历缓冲区
    for (auto it = asmBuffer.begin(); it != asmBuffer.end(); ) {
        // 获取下一条指令迭代器
        auto nextIt = std::next(it);
        if (nextIt == asmBuffer.end()) break;

        std::string curr = *it;
        std::string next = *nextIt;
        ...
        // _L1:
        // 动作: 删除 j _L1
        // 判断依据: curr 以 "    j " 开头，且跳转目标与 next 的 Label 匹配
        if (curr.substr(0, 6) == "    j ") {
            std::string jumpTarget = curr.substr(6); // 获取 "_L1"
            std::string nextLabel = next;
            if (!nextLabel.empty() && nextLabel.back() == ':') {
                nextLabel.pop_back(); // 去掉冒号 "_L1"
                if (jumpTarget == nextLabel) {
                    // 命中！删除当前跳转指令
                    it = asmBuffer.erase(it);
                    continue; // 重新检查当前位置（即原来的 next）
                }
            }
        }
        ...
        // 场景:
        //     sw $t0, -4($fp)
        //     lw $t0, -4($fp)  <-- 冗余，因为 $t0 的值没变
        // 动作: 删除 lw 指令
        // 判断依据:
        // 1. curr 是 sw, next 是 lw
        // 2. 操作数完全一致 (字符串后缀相同)
        // 3. 确保中间没有 Label (迭代器紧邻，自然满足)
        if (curr.substr(0, 7) == "    sw " && next.substr(0, 7) == "    lw ") {
            // sw $reg, addr
            // lw $reg, addr
            // 从第 7 个字符开始比较后续字符串是否完全一致
            if (curr.substr(7) == next.substr(7)) {
                // 命中！删除下一条 lw 指令
                asmBuffer.erase(nextIt);
                // 不移动 it，因为删除后后面的指令移上来了，可能还能优化
                continue;
            }
        }
        ...
        // 场景: move $t0, $t0
        // 动作: 删除
        if (curr.substr(0, 9) == "    move ") {
            // 格式: "    move $r1, $r2"
            size_t commaPos = curr.find(',');
            if (commaPos != std::string::npos) {
                // 提取 r1 ($t0) 和 r2 ($t0)
                // substr(start, length)
                // r1 从下标 9 开始，长度 = commaPos - 9
                // r2 从下标 commaPos + 2 开始 (跳过 ", ")
                std::string r1 = curr.substr(9, commaPos - 9);
                std::string r2 = curr.substr(commaPos + 2);
                if (r1 == r2) {
                    it = asmBuffer.erase(it);
                    continue;
                }
...
void MIPSGenerator::flushAsmBuffer() {
    // 1. 先执行优化
    peepholeOptimize();

    // 2. 将结果写入文件
    for (const auto& line : asmBuffer) {
        fout << line << "\n";
    }
